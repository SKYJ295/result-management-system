...
class ResultClass:
    def __init__(self,root):
        ...
        #button
        btn_add=Button(self.root,text='Submit',font=("times new roman",15,"bold"),bg="lightgreen",fg="white",activebackground="lightgreen",cursor="hand2",command=self.add).place(x=300,y=420,width=120,height=35)
        btn_add=Button(self.root,text='Clear',font=("times new roman",15,"bold"),bg="lightgray",fg="white",activebackground="lightgray",cursor="hand2",command=self.clear).place(x=430,y=420,width=120,height=35)

        #image
        self.bg_img=Image.open("RMS/images/result.jpg")
        ...
        try:
            if self.var_name.get()=="":
                ...
                if row!=None:
                    ...
                    con.commit()
                    messagebox.showinfo("Sucess","Result Added Successfully",parent=self.root)
        except Exception as ex:
            messagebox.showerror("Error",f"Error due to {str(ex)}")
